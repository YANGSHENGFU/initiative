...
public class SortTools {
     ...
     * @return
     */
    public static TreeMap<String , String > getSortMap( List<String> key , Map<String,String> vula ){
        if( null == key || null== vula || key.size() == 0 || vula.size()==0 ){
            return null;
        }
        TreeMap<String , String > paraMap = new TreeMap<>();
        Iterator iterator = key.iterator();
        while (iterator.hasNext()){
            String a = (String) iterator.next();  //从keyList中获取key值
            paraMap.put(a , vula.get(a));         //保存在TreeMap集合中
        }
     ...
     * @return
     */
    public static List<String> listSort(Set<String> list) {
